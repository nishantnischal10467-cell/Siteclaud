...
import type { RtfConversionResponse } from "@/types/conversion";

async function convertRtf(file: File) {
  ...
  if (!response.ok) {
    throw new Error(data.error ?? "RTF conversion failed.");
  }

  return data as RtfConversionResponse;
}
...
export function RtfMarkdownConverter() {
  const inputRef = useRef<HTMLInputElement>(null);
  const [file, setFile] = useState<File | null>(null);
  const [dragging, setDragging] = useState(false);
  const [result, setResult] = useState<RtfConversionResponse | null>(null);

  const mutation = useMutation({
    mutationFn: convertRtf,
    onSuccess: (data) => {
      setResult(data);
      toast.success("RTF converted to Markdown.");
    },
    onError: (error) => toast.error(error instanceof Error ? error.message : "RTF conversion failed."),
  });

  const preview = useMemo(
    () => result?.markdown.slice(0, 2400) ?? "Upload an RTF document to convert rich text, paragraphs, bold, italics, and document spacing into clean Markdown.",
    [result],
  );
  ...
  function selectFile(nextFile?: File) {
    ...
    if (!nextFile.name.toLowerCase().endsWith(".rtf")) {
      toast.error("Please choose an RTF file.");
      return;
    }

    if (nextFile.size > 2 * 1024 * 1024) {
      toast.error("RTF must be 2MB or smaller.");
      return;
    }
  ...
  function download(ext: "md" | "txt") {
    ...
    link.download = `${result.title.replace(/[^a-z0-9]+/gi, "-").toLowerCase() || "siteclaud-rtf-export"}.${ext}`;
    link.click();
    ...
    <div className="grid gap-8">
      <Card className="glass-panel rounded-3xl">
        <CardHeader className="gap-3 text-center">
          ...
          <h1 className="text-balance text-4xl font-semibold tracking-tight md:text-6xl">Convert RTF to Markdown</h1>
          <CardDescription className="mx-auto max-w-2xl text-base leading-7">
            Upload any RTF document and convert it into clean Markdown for documentation, note-taking, content migration, and AI knowledge workflows.
          </CardDescription>
        </CardHeader>
        <CardContent className="grid gap-5">
          ...
          >
            <div className="flex max-w-md flex-col items-center gap-4">
              ...
              <div>
                <p className="text-lg font-semibold">{file ? file.name : "Choose an RTF file or drag & drop it here"}</p>
                <p className="mt-2 text-sm text-muted-foreground">.rtf format supported up to 2MB. Rich text styles are normalized into Markdown-friendly structure.</p>
              </div>
              {file ? <Badge variant="secondary">{formatBytes(file.size)}</Badge> : null}
            </div>
          </button>
          <input ref={inputRef} type="file" accept=".rtf,application/rtf,text/rtf" className="hidden" onChange={(event) => selectFile(event.target.files?.[0])} />
