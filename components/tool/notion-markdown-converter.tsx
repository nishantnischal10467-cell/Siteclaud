...
async function convertNotion(url: string) {
  const response = await fetch("/api/convert-notion", {
    method: "POST",
  ...
  if (!response.ok) {
    throw new Error(data.error ?? "Notion conversion failed.");
  }

  return data as ConversionResponse;
}

export function NotionMarkdownConverter() {
  const [url, setUrl] = useState("https://www.notion.site/help");
  const [result, setResult] = useState<ConversionResponse | null>(null);

  const mutation = useMutation({
    mutationFn: convertNotion,
    onSuccess: (data) => {
      setResult(data);
      toast.success("Notion page converted to Markdown.");
    },
    onError: (error) => toast.error(error instanceof Error ? error.message : "Notion conversion failed."),
  });

  const preview = useMemo(
    () => result?.markdown.slice(0, 2400) ?? "Enter a public Notion page URL to convert it into clean Markdown for documentation, archiving, AI context, and migration workflows.",
    [result],
  );
  ...
  function download(ext: "md" | "txt") {
    ...
    link.download = `${result.title.replace(/[^a-z0-9]+/gi, "-").toLowerCase() || "siteclaud-notion-export"}.${ext}`;
    link.click();
    ...
    <div className="grid gap-8">
      <Card className="glass-panel rounded-3xl">
        <CardHeader className="gap-3 text-center">
          ...
          <h1 className="text-balance text-4xl font-semibold tracking-tight md:text-6xl">Convert Notion to Markdown</h1>
          <CardDescription className="mx-auto max-w-2xl text-base leading-7">
            Enter any public Notion page URL and convert it into clean Markdown for documentation, content migration, archiving, and AI knowledge workflows.
          </CardDescription>
        </CardHeader>
        <CardContent className="grid gap-4">
          <div className="flex flex-col gap-3 md:flex-row">
            <Input value={url} onChange={(event) => setUrl(event.target.value)} placeholder="https://your-workspace.notion.site/Public-page" className="h-12 text-base" />
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-border bg-muted/40 p-4 text-sm text-muted-foreground">
            <span>Note: the Notion page must be public to be converted.</span>
      ...
      <div className="grid gap-6 lg:grid-cols-[0.8fr_1.2fr]">
        <Card className="rounded-3xl">
          <CardHeader>
            <CardTitle>Notion page metadata</CardTitle>
          ...
          <CardContent className="grid gap-4">
            {[
              ...
              ["URL", result?.url ?? (url || "Waiting for public page")],
