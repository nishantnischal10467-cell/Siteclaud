...
import type { PdfConversionResponse } from "@/types/conversion";
...
async function convertPdf(file: File) {
  ...
  return data as PdfConversionResponse;
}
...
export function PdfMarkdownConverter() {
  ...
  const [result, setResult] = useState<PdfConversionResponse | null>(null);

  const mutation = useMutation({
    mutationFn: convertPdf,
    onSuccess: (data) => {
      setResult(data);
      toast.success("PDF converted to Markdown.");
    },
    onError: (error) => toast.error(error instanceof Error ? error.message : "PDF conversion failed."),
  });

  const preview = useMemo(
    () => result?.markdown.slice(0, 2400) ?? "Upload a PDF to extract text and convert it into clean Markdown for AI workflows, notes, docs, and retrieval pipelines.",
    [result],
  );
  ...
  function download(ext: "md" | "txt") {
    ...
    link.download = `${result.title.replace(/[^a-z0-9]+/gi, "-").toLowerCase() || "siteclaud-pdf-export"}.${ext}`;
    link.click();
    ...
    <div className="grid gap-8">
      <Card className="glass-panel rounded-3xl">
        <CardHeader className="gap-3 text-center">
          ...
          <h1 className="text-balance text-4xl font-semibold tracking-tight md:text-6xl">Convert PDF to Markdown</h1>
          <CardDescription className="mx-auto max-w-2xl text-base leading-7">
            Upload a PDF and turn it into clean Markdown for AI systems, documentation, research notes, and content workflows.
          </CardDescription>
        </CardHeader>
        <CardContent className="grid gap-5">
          ...
          >
            <div className="flex max-w-md flex-col items-center gap-4">
              ...
              <div>
                <p className="text-lg font-semibold">{file ? file.name : "Upload or drag & drop your PDF"}</p>
                <p className="mt-2 text-sm text-muted-foreground">Supports text-based PDFs up to 15MB. Scanned image PDFs need OCR before conversion.</p>
              </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[0.8fr_1.2fr]">
        <Card className="rounded-3xl">
          <CardHeader>
            ...
            <CardDescription>Document stats for planning AI context and exports.</CardDescription>
          </CardHeader>
          <CardContent className="grid gap-4">
            {[
              ["File", result?.fileName ?? file?.name ?? "No PDF selected"],
              ["Title", result?.title ?? "Waiting for conversion"],
              ["Pages", result ? result.pageCount.toLocaleString() : "0"],
